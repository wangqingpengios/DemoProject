...
// icon与placeholder间距
static CGFloat const iconSpacing = 30.0;
// 占位文字的字体大小
...
- (void)layoutSubviews {
    ...
    for (UIView *view in [self.subviews lastObject].subviews) {
        if ([view isKindOfClass:[UITextField class]]) {
            ...
            // 重设field的frame
            field.frame = CGRectMake(20.0, 10.5, self.frame.size.width-30.0, self.frame.size.height-15.0);
            [field setBackgroundColor:[UIColor whiteColor]];
            field.textColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1];
            
            field.borderStyle = UITextBorderStyleRoundedRect;
            field.layer.cornerRadius = 11.0f;
            ...
            if (@available(iOS 11.0, *)) {
                // 先默认居中placeholder
                [self setPositionAdjustment:UIOffsetMake((field.frame.size.width-self.placeholderWidth)/2-10, 0) forSearchBarIcon:UISearchBarIconSearch];
            }
        }
    }
}
