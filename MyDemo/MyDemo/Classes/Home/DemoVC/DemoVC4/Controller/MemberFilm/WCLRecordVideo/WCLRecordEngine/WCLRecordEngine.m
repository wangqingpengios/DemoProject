        ...
        if (self.isCapturing) {
            ...
            dispatch_async(_captureQueue, ^{
                [self.recordEncoder finishWithCompletionHandler:^{
                    ...
                    if ([self.delegate respondsToSelector:@selector(recordProgress:time:)]) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [self.delegate recordProgress:self.currentRecordTime/self.maxRecordTime time:self.currentRecordTime];
                        });
                    }
...
- (void)changeMovToMp4:(NSURL *)mediaURL dataBlock:(void (^)(UIImage *movieImage))handler {
    ...
    self.videoName=[self getUploadFile_type:@"video" fileType:@"mp4"];
    self.videoPath = [basePath stringByAppendingPathComponent:_videoName];
