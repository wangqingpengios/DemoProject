...
- (WCLRecordEngine *)recordEngine {
    if (_recordEngine == nil) {
        ...
        _recordEngine.maxRecordTime = 15;
    ...
    if ([[info objectForKey:UIImagePickerControllerMediaType] isEqualToString:(NSString*)kUTTypeMovie]) {
        ...
        if ([suffixName isEqualToString:@"MOV"]) {
            ...
            [self.recordEngine changeMovToMp4:videoUrl dataBlock:^(UIImage *movieImage) {
                ...
                [weakSelf.moviePicker dismissViewControllerAnimated:NO completion:^{
...
- (void)recordProgress:(CGFloat)progress time:(int)time {
    if (progress >= 1) {
    ...
    if (time>119) {
        ...
        if ((time-60)<10) {
            strtime=[NSString stringWithFormat:@"01:0%d",time-60];
        }else{
            strtime=[NSString stringWithFormat:@"01:%d",time-60];
        }
    }else{
        if (time<10) {
            strtime=[NSString stringWithFormat:@"00:0%d",time];
        }else{
            strtime=[NSString stringWithFormat:@"00:%d",time];
        }
    }
    _labTime.text=strtime;
